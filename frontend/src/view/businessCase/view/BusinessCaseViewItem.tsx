...
function BusinessCaseViewItem(props) {
      ...
      return (
        <div key={record.id}>
          <Link
            ...
            to={`/business-case/${record.id}`}
          >
            {record.businessNeeds}
          </Link>
        </div>
      );
    }

    return <div key={record.id}>{record.businessNeeds}</div>;
  };
...
BusinessCaseViewItem.propTypes = {
  label: PropTypes.string,
  value: PropTypes.any,
};

export default BusinessCaseViewItem;
