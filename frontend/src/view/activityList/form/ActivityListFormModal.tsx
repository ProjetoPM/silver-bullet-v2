...
import ActivityListForm from 'src/view/activityList/form/ActivityListForm';
import ActivityListService from 'src/modules/activityList/activityListService';

function ActivityListFormModal(props) {
  const [saveLoading, setSaveLoading] = useState(false);

  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await ActivityListService.create(data);
      const record = await ActivityListService.find(id);
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            <div className="flex items-start justify-between p-5 rounded-t">
              <div className="text-lg font-semibold">
                {i18n('entities.activityList.new.title')}
              </div>
              <button
                type="button"
...
export default ActivityListFormModal;
