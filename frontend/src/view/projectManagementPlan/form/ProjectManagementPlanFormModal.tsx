...
function ProjectManagementPlanFormModal(props) {
  const [saveLoading, setSaveLoading] = useState(false);

  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await ProjectManagementPlanService.create(data);
      const record = await ProjectManagementPlanService.find(id);
...
export default ProjectManagementPlanFormModal;
