...
function ResourceBreakdownStructureToolbar(props) {
  const [
  ...
  const hasPermissionToCreate = useSelector(
    resourceBreakdownStructureSelectors.selectPermissionToCreate,
  );
  const hasPermissionToImport = useSelector(
    resourceBreakdownStructureSelectors.selectPermissionToImport,
  );
    ...
    if (disabled) {
      return (
        <span
          ...
          data-for="resource-breakdown-structure-list-toolbar-export-tooltip"
        >
          {button}
          <ReactTooltip id="resource-breakdown-structure-list-toolbar-export-tooltip" />
        </span>
      );
    }
    ...
    if (disabled) {
      return (
        <span
          ...
          data-tip-disable={!disabled}
          data-for="resource-breakdown-structure-list-toolbar-destroy-tooltip"
        >
          {button}
          <ReactTooltip id="resource-breakdown-structure-list-toolbar-destroy-tooltip" />
        </span>
      );
    }
  ...
  return (
    <div className="mb-4">
      {hasPermissionToCreate && (
        <Link to="/resource-breakdown-structure/new">
          <button
            className="mb-2 mr-2 text-sm disabled:opacity-50 disabled:cursor-default px-4 py-2 tracking-wide text-white transition-colors duration-200 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600"
            type="button"
          >
      ...
      {hasPermissionToImport && (
        <Link to="/resource-breakdown-structure/importer">
          <button
            className="mb-2 mr-2 text-sm disabled:opacity-50 disabled:cursor-default px-4 py-2 tracking-wide text-white transition-colors duration-200 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600"
            type="button"
          >
      ...
      {hasPermissionToAuditLogs && (
        <Link to="/audit-logs?entityNames=resourceBreakdownStructure">
          <button
            className="mb-2 mr-2 text-sm disabled:opacity-50 disabled:cursor-default px-4 py-2 tracking-wide dark:border-gray-800 dark:bg-gray-800 dark:hover:bg-gray-600 dark:text-white text-gray-700 border border-gray-300 transition-colors duration-200 transform bg-white rounded-md hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
            type="button"
          >
...
export default ResourceBreakdownStructureToolbar;
