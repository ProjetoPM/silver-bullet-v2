...
import FinalReportForm from 'src/view/finalReport/form/FinalReportForm';
import FinalReportService from 'src/modules/finalReport/finalReportService';

function FinalReportFormModal(props) {
  const [saveLoading, setSaveLoading] = useState(false);

  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await FinalReportService.create(data);
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            <div className="flex items-start justify-between p-5 rounded-t">
              <div className="text-lg font-semibold">
                {i18n('entities.finalReport.new.title')}
              </div>
              <button
                type="button"
...
export default FinalReportFormModal;
