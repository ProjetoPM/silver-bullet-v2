...
import ProcurementStatementWorkRegisterForm from 'src/view/procurementStatementWorkRegister/form/ProcurementStatementWorkRegisterForm';
...
function ProcurementStatementWorkRegisterFormModal(props) {
  const [saveLoading, setSaveLoading] = useState(false);

  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await ProcurementStatementWorkRegisterService.create(data);
      const record = await ProcurementStatementWorkRegisterService.find(id);
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            <div className="flex items-start justify-between p-5 rounded-t">
              <div className="text-lg font-semibold">
                {i18n('entities.procurementStatementWorkRegister.new.title')}
              </div>
              <button
                type="button"
            ...
            <div className="relative p-6 flex-auto overflow-y-scroll">
              <ProcurementStatementWorkRegisterForm
...
export default ProcurementStatementWorkRegisterFormModal;
